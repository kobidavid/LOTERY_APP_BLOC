...
class OperatorConsoleRepository {
  ...
  Future<void> updateDispatchStatus({
    ...
    debugPrint(
      ...
      'formRef=${formRef.path}',
    );

    try {
      await _firestore.runTransaction((transaction) async {
        debugPrint('[RepoDispatch] transaction: reading formRef=${formRef.path}');
        ...
        debugPrint(
          '[RepoDispatch] formSnapshot exists=${formSnapshot.exists} '
        ...
        debugPrint(
          '[RepoDispatch] writing formRef=${formRef.path} '
        ...
        debugPrint('[RepoDispatch] transaction: reading groupRef=${groupRef.path}');
        final DocumentSnapshot<Map<String, dynamic>> groupSnapshot =
            await transaction.get(groupRef);

        debugPrint(
          '[RepoDispatch] groupSnapshot exists=${groupSnapshot.exists}',
        );

        if (groupSnapshot.exists) {
          debugPrint(
            '[RepoDispatch] writing groupRef=${groupRef.path} '
            'payload=$dispatchUpdate',
          );
        ...
        debugPrint(
          '[RepoDispatch] participantUserIds=$participantUserIds',
        );
        ...
        for (final String userId in participantUserIds) {
          ...
          final Map<String, dynamic> submittedGroupUpdate = <String, dynamic>{
            ...dispatchUpdate,
            'groupStatus': 'submitted',
          };

          debugPrint(
            '[RepoDispatch] writing submittedGroupRef=${submittedGroupRef.path} '
            'payload=$submittedGroupUpdate',
          );

          transaction.set(
            submittedGroupRef,
            submittedGroupUpdate,
            SetOptions(merge: true),
          );
        }
      });

      debugPrint('[RepoDispatch] success');
    } on FirebaseException catch (e, st) {
      debugPrint(
        '[RepoDispatch] FIREBASE ERROR '
        'code=${e.code} message=${e.message}',
      );
      debugPrint('$st');
      rethrow;
    } catch (e, st) {
      debugPrint('[RepoDispatch] GENERIC ERROR: $e');
      debugPrint('$st');
      rethrow;
    }
  }
